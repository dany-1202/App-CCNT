...
ctrlCCNT.controller('configController', function ($route, PromiseDAO, $timeout, $rootScope, $mdDialog, $scope, $http, $location, $mdpDatePicker, $mdpTimePicker, SessionService, NotifService, Const, State, Postaux, DateFactory, Popover) {
	...
	*/
	...
	var insertHorairePreconfigure = function (nom, id) {
		for (var cpt = 0; cpt < $scope.prehours.length; cpt++) {
			...
			if (objPreHour.dep.name == nom) {
				var data = { 'hpr_nom': objPreHour.title, 'hpr_dep_id': id, 'user_id': SessionService.get('user_id'), 'user_token': SessionService.get('user_token') };
				var $res = $http.post("assets/php/insertHorairePreconfigureAPI.php", data);
				$res.then(function (msgPre) {
					console.log(cpt);
					//insertJourPreConfigure(msgPre.data, );
				});
			}
		};
	}
	...
	this.saveConfiguration = function () {
		...
		// $res.then(function (msgEta) {
		//  	/* Insertion des horaires */
		//  	var idEstablishment = msgEta.data;
		...
		// 		$res.then(function (msgDep) {
		// 			insertHorairePreconfigure(msgDep.config.data.nom, msgDep.data);
		// 		});
		// 	};
		...
		/* Insertion des horaires types préconfiguré  */
		/*	for (var i = 0; i < $scope.prehours.length; i++) {
				var obj = $scope.prehours[i];
				var data = { 'hpr_libelle': obj.title, 'hpr_dep_id': obj.dep.id, 'user_id': SessionService.get('user_id'), 'user_token': SessionService.get('user_token') };
				var $res = $http.post("assets/php/insertHorairePreconfigureAPI.php", data);
